...
{
  warningWithoutStack = function (format) {
...
function setComponentTree(getFiberCurrentPropsFromNodeImpl, getInstanceFromNodeImpl, getNodeFromInstanceImpl) {
...

  {
    if (!getNodeFromInstance || !getInstanceFromNode) {
      warningWithoutStack$1('EventPluginUtils.setComponentTree(...): Injected ' + 'module is missing getNodeFromInstance or getInstanceFromNode.');
    }
  }
}
...

{
  validateEventDispatches = function (event) {
    ...
    if (instancesIsArr !== listenersIsArr || instancesLen !== listenersLen) {
      warningWithoutStack$1('EventPluginUtils: Invalid `event`.');
    }
  };
}
/**
...

{
  warning = function (format) {
    ...
    var stack = ReactDebugCurrentFrame.getStackAddendum(); // eslint-disable-next-line react-internal/warning-args

    for (var _len = arguments.length, args = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
      args[_key - 1] = arguments[_key];
    }

    warningWithoutStack$1.apply(void 0, [format + '%s'].concat(args, [stack]));
  };
}
      ...
      if (lazyComponent._status === Pending) {
...

        {
          if (defaultExport === undefined) {
            warning$1('lazy: Expected the result of a dynamic import() call. ' + 'Instead received: %s\n\nYour code should look like: \n  ' + "const MyComponent = lazy(() => import('./MyComponent'))", moduleObject);
          }
        }
...
function getComponentName(type) {
...

  {
    if (typeof type.tag === 'number') {
      warningWithoutStack$1('Received an unexpected object in getComponentName(). ' + 'This is likely a bug in React. Please file an issue.');
    }
  }
...
function validateEventValue(eventValue) {
  if (typeof eventValue === 'object' && eventValue !== null) {
    ...
    var showWarning = function (name) {
      {
        warning$1('%s is not available on event objects created from event responder modules (React Flare). ' + 'Try wrapping in a conditional, i.e. `if (event.type !== "press") { event.%s }`', name, name);
      }
    };
...
function isAttributeNameSafe(attributeName) {
  ...
  {
    warning$1('Invalid attribute name: `%s`', attributeName);
  }
...
function sanitizeURL(url) {
  if (disableJavaScriptURLs) {
    ...
    if (!didWarn && isJavaScriptProtocol.test(url)) {
      didWarn = true;
      warning$1('A future version of React will block javascript: URLs as a security precaution. ' + 'Use event handlers instead if you can. If you need to generate unsafe HTML try ' + 'using dangerouslySetInnerHTML instead. React was passed %s.', JSON.stringify(url));
    }
  }
}
...

{
  ...
  var propTypes = {
    value: function (props, propName, componentName) {
      if (hasReadOnlyValue[props.type] || props.onChange || props.readOnly || props.disabled || props[propName] == null || enableFlareAPI && props.DEPRECATED_flareListeners) {
        return null;
      }
    ...
    checked: function (props, propName, componentName) {
      if (props.onChange || props.readOnly || props.disabled || props[propName] == null || enableFlareAPI && props.DEPRECATED_flareListeners) {
        return null;
      }
...
function updateWrapper(element, props) {
...

  {
    ...
    if (node._wrapperState.controlled && !controlled && !didWarnControlledToUncontrolled) {
      warning$1('A component is changing a controlled input of type %s to be uncontrolled. ' + 'Input elements should not switch from controlled to uncontrolled (or vice versa). ' + 'Decide between using a controlled or uncontrolled input ' + 'element for the lifetime of the component. More info: https://fb.me/react-controlled-components', props.type);
...
function validateProps(element, props) {
  {
    ...
    if (props.selected != null && !didWarnSelectedSetOnOption) {
      warning$1('Use the `defaultValue` or `value` props on <select> instead of ' + 'setting `selected` on <option>.');
...
function checkSelectPropTypes(props) {
  {
    ...
    for (var i = 0; i < valuePropNames.length; i++) {
      ...
      if (props.multiple && !isArray) {
        warning$1('The `%s` prop supplied to <select> must be an array if ' + '`multiple` is true.%s', propName, getDeclarationErrorAddendum());
      } else if (!props.multiple && isArray) {
        warning$1('The `%s` prop supplied to <select> must be a scalar ' + 'value if `multiple` is false.%s', propName, getDeclarationErrorAddendum());
      }
    }
  }
}
...
function initWrapperState$1(element, props) {
...

  {
    if (props.value !== undefined && props.defaultValue !== undefined && !didWarnValueDefaultValue$1) {
      warning$1('Select elements must be either controlled or uncontrolled ' + '(specify either the value prop, or the defaultValue prop, but not ' + 'both). Decide between using a controlled or uncontrolled select ' + 'element and remove one of these props. More info: ' + 'https://fb.me/react-controlled-components');
...
function initWrapperState$2(element, props) {
...

  {
    ...
    if (props.value !== undefined && props.defaultValue !== undefined && !didWarnValDefaultVal) {
      warning$1('%s contains a textarea with both value and defaultValue props. ' + 'Textarea elements must be either controlled or uncontrolled ' + '(specify either the value prop, or the defaultValue prop, but not ' + 'both). Decide between using a controlled or uncontrolled textarea ' + 'and remove one of these props. More info: ' + 'https://fb.me/react-controlled-components', getCurrentFiberOwnerNameInDevOrNull() || 'A component');
  ...
  if (initialValue == null) {
    ...
    if (children != null) {
      {
        warning$1('Use the `defaultValue` or `value` props instead of setting ' + 'children on <textarea>.');
      }
  ...
  if (node.namespaceURI === Namespaces.svg) {
    {
      if (enableTrustedTypesIntegration) {
        ...
        if (typeof trustedTypes !== 'undefined') {
          warning$1("Using 'dangerouslySetInnerHTML' in an svg element with " + 'Trusted Types enabled in an Internet Explorer will cause ' + 'the trusted value to be converted to string. Assigning string ' + "to 'innerHTML' will throw an error if Trusted Types are enforced. " + "You can try to wrap your svg element inside a div and use 'dangerouslySetInnerHTML' " + 'on the enclosing div instead.');
        }
      }
    }
...
function accumulateDirectionalDispatches(inst, phase, event) {
  {
    if (!inst) {
      warningWithoutStack$1('Dispatching inst must not be null');
    }
  }
      ...
      default:
        {
          if (knownHTMLTopLevelTypes.indexOf(topLevelType) === -1) {
            warningWithoutStack$1('SimpleEventPlugin: Unhandled event type, `%s`. This warning ' + 'is likely caused by a bug in React. Please file an issue.', topLevelType);
          }
        } // HTML Events
...

{
  ...
  var warnHyphenatedStyleName = function (name) {
    ...
    warning$1('Unsupported style property %s. Did you mean %s?', name, // As Andi Smith suggests
  ...
  var warnStyleValueIsNaN = function (name, value) {
    ...
    warning$1('`NaN` is an invalid value for the `%s` css style property.', name);
  };
  ...
  var warnStyleValueIsInfinity = function (name, value) {
    ...
    warning$1('`Infinity` is an invalid value for the `%s` css style property.', name);
  };
...
function validateShorthandPropertyCollisionInDev(styleUpdates, nextStyles) {
  {
    ...
    for (var key in expandedUpdates) {
      ...
      if (correctOriginalKey && originalKey !== correctOriginalKey) {
        ...
        warning$1('%s a style property during rerender (%s) when a ' + 'conflicting property is set (%s) can lead to styling bugs. To ' + "avoid this, don't mix shorthand and non-shorthand properties " + 'for the same value; instead, replace the shorthand with ' + 'separate values.', isValueEmpty(styleUpdates[originalKey]) ? 'Removing' : 'Updating', originalKey, correctOriginalKey);
      }
    }
  }
}
...
function assertValidProps(tag, props) {
...

  {
    if (!props.suppressContentEditableWarning && props.contentEditable && props.children != null) {
      warning$1('A component is `contentEditable` and contains `children` managed by ' + 'React. It is now your responsibility to guarantee that none of ' + 'those nodes are unexpectedly modified or duplicated. This is ' + 'probably not intentional.');
    }
  }
...
function validateProperty(tagName, name) {
  {
    ...
    if (rARIACamel.test(name)) {
      ...
      if (correctName == null) {
        warning$1('Invalid ARIA attribute `%s`. ARIA attributes follow the pattern aria-* and must be lowercase.', name);
      ...
      if (name !== correctName) {
        warning$1('Invalid ARIA attribute `%s`. Did you mean `%s`?', name, correctName);
        warnedProperties[name] = true;
        return true;
      }
    }
    ...
    if (rARIA.test(name)) {
      ...
      if (name !== standardName) {
        warning$1('Unknown ARIA attribute `%s`. Did you mean `%s`?', name, standardName);
        warnedProperties[name] = true;
        return true;
      }
    }
...
function warnInvalidARIAProps(type, props) {
  {
    ...
    if (invalidProps.length === 1) {
      warning$1('Invalid aria prop %s on <%s> tag. ' + 'For details, see https://fb.me/invalid-aria-prop', unknownPropString, type);
    } else if (invalidProps.length > 1) {
      warning$1('Invalid aria props %s on <%s> tag. ' + 'For details, see https://fb.me/invalid-aria-prop', unknownPropString, type);
    }
  }
}
...
function validateProperties$1(type, props) {
  {
    ...
    if (props != null && props.value === null && !didWarnValueNull) {
      ...
      if (type === 'select' && props.multiple) {
        warning$1('`value` prop on `%s` should not be null. ' + 'Consider using an empty array when `multiple` is set to `true` ' + 'to clear the component or `undefined` for uncontrolled components.', type);
      } else {
        warning$1('`value` prop on `%s` should not be null. ' + 'Consider using an empty string to clear the component or `undefined` ' + 'for uncontrolled components.', type);
      }
    }
  }
}
...

{
  ...
  validateProperty$1 = function (tagName, name, value, canUseEventSystem) {
    ...
    if (lowerCasedName === 'onfocusin' || lowerCasedName === 'onfocusout') {
      warning$1('React uses onFocus and onBlur instead of onFocusIn and onFocusOut. ' + 'All React events are normalized to bubble, so onFocusIn and onFocusOut ' + 'are not needed/supported by React.');
    ...
    if (canUseEventSystem) {
      ...
      if (registrationName != null) {
        warning$1('Invalid event handler property `%s`. Did you mean `%s`?', name, registrationName);
        warnedProperties$1[name] = true;
        return true;
      }

      if (EVENT_NAME_REGEX.test(name)) {
        warning$1('Unknown event handler property `%s`. It will be ignored.', name);
        warnedProperties$1[name] = true;
        return true;
      }
      ...
      if (INVALID_EVENT_NAME_REGEX.test(name)) {
        warning$1('Invalid event handler property `%s`. ' + 'React events use the camelCase naming convention, for example `onClick`.', name);
      }
    ...
    if (lowerCasedName === 'innerhtml') {
      warning$1('Directly setting property `innerHTML` is not permitted. ' + 'For more information, lookup documentation on `dangerouslySetInnerHTML`.');
      warnedProperties$1[name] = true;
      return true;
    }

    if (lowerCasedName === 'aria') {
      warning$1('The `aria` attribute is reserved for future use in React. ' + 'Pass individual `aria-` attributes instead.');
      warnedProperties$1[name] = true;
      return true;
    }

    if (lowerCasedName === 'is' && value !== null && value !== undefined && typeof value !== 'string') {
      warning$1('Received a `%s` for a string attribute `is`. If this is expected, cast ' + 'the value to a string.', typeof value);
      warnedProperties$1[name] = true;
      return true;
    }

    if (typeof value === 'number' && isNaN(value)) {
      warning$1('Received NaN for the `%s` attribute. If this is expected, cast ' + 'the value to a string.', name);
      warnedProperties$1[name] = true;
      return true;
    }
    ...
    if (possibleStandardNames.hasOwnProperty(lowerCasedName)) {
      ...
      if (standardName !== name) {
        warning$1('Invalid DOM property `%s`. Did you mean `%s`?', name, standardName);
        warnedProperties$1[name] = true;
        return true;
      }
      ...
      warning$1('React does not recognize the `%s` prop on a DOM element. If you ' + 'intentionally want it to appear in the DOM as a custom ' + 'attribute, spell it as lowercase `%s` instead. ' + 'If you accidentally passed it from a parent component, remove ' + 'it from the DOM element.', name, lowerCasedName);
      warnedProperties$1[name] = true;
      return true;
    }
    ...
    if (typeof value === 'boolean' && shouldRemoveAttributeWithWarning(name, value, propertyInfo, false)) {
      if (value) {
        warning$1('Received `%s` for a non-boolean attribute `%s`.\n\n' + 'If you want to write it to the DOM, pass a string instead: ' + '%s="%s" or %s={value.toString()}.', value, name, name, value, name);
      } else {
    ...
    if ((value === 'false' || value === 'true') && propertyInfo !== null && propertyInfo.type === BOOLEAN) {
      warning$1('Received the string `%s` for the boolean attribute `%s`. ' + '%s ' + 'Did you mean %s={%s}?', value, name, value === 'false' ? 'The browser will interpret it as a truthy value.' : 'Although this works, it will not work as expected if you pass the string "false".', name, value);
      warnedProperties$1[name] = true;
      return true;
    }
...
var warnUnknownProperties = function (type, props, canUseEventSystem) {
  {
    ...
    if (unknownProps.length === 1) {
      warning$1('Invalid value for prop %s on <%s> tag. Either remove it from the element, ' + 'or pass a string or number value to keep it in the DOM. ' + 'For details, see https://fb.me/react-attribute-behavior', unknownPropString, type);
    } else if (unknownProps.length > 1) {
      warning$1('Invalid values for props %s on <%s> tag. Either remove them from the element, ' + 'or pass a string or number value to keep them in the DOM. ' + 'For details, see https://fb.me/react-attribute-behavior', unknownPropString, type);
    }
  }
};
...

{
  ...
  warnForTextDifference = function (serverText, clientText) {
    ...
    warningWithoutStack$1('Text content did not match. Server: "%s" Client: "%s"', normalizedServerText, normalizedClientText);
  };
  ...
  warnForPropDifference = function (propName, serverValue, clientValue) {
    ...
    didWarnInvalidHydration = true;
    warningWithoutStack$1('Prop `%s` did not match. Server: %s Client: %s', propName, JSON.stringify(normalizedServerValue), JSON.stringify(normalizedClientValue));
  };
  ...
  warnForExtraAttributes = function (attributeNames) {
    ...
    warningWithoutStack$1('Extra attributes from the server: %s', names);
  };

  warnForInvalidEventListener = function (registrationName, listener) {
    if (listener === false) {
      warning$1('Expected `%s` listener to be a function, instead got `false`.\n\n' + 'If you used to conditionally omit it with %s={condition && value}, ' + 'pass %s={condition ? value : undefined} instead.', registrationName, registrationName, registrationName);
    } else {
      warning$1('Expected `%s` listener to be a function, instead got a value of `%s` type.', registrationName, typeof listener);
    }
...
function setInitialDOMProperties(tag, domElement, rootContainerElement, nextProps, isCustomComponentTag) {
  for (var propKey in nextProps) {
    ...
    } else if (enableFlareAPI && propKey === DEPRECATED_flareListeners || propKey === SUPPRESS_CONTENT_EDITABLE_WARNING || propKey === SUPPRESS_HYDRATION_WARNING$1) {// Noop
  ...
  if (namespaceURI === HTML_NAMESPACE) {
    {
      ...
      if (!isCustomComponentTag && type !== type.toLowerCase()) {
        warning$1('<%s /> is using incorrect casing. ' + 'Use PascalCase for React components, ' + 'or lowercase for HTML elements.', type);
      }
    }
    ...
    if (type === 'script') {
...

      {
        if (enableTrustedTypesIntegration && !didWarnScriptTags) {
          warning$1('Encountered a script tag while rendering React component. ' + 'Scripts inside React components are never executed when rendering ' + 'on the client. Consider using template tag instead ' + '(https://developer.mozilla.org/en-US/docs/Web/HTML/Element/template).');
...
function setInitialProperties(domElement, tag, rawProps, rootContainerElement) {
...

  {
    ...
    if (isCustomComponentTag && !didWarnShadyDOM && domElement.shadyRoot) {
      warning$1('%s is using shady DOM. Using shady DOM with React can ' + 'cause things to break subtly.', getCurrentFiberOwnerNameInDevOrNull() || 'A component');
...
function diffHydratedProperties(domElement, tag, rawProps, parentNamespace, rootContainerElement) {
  ...
  for (var propKey in rawProps) {
    ...
    typeof isCustomComponentTag === 'boolean') {
      ...
      } else if (enableFlareAPI && propKey === DEPRECATED_flareListeners || propKey === SUPPRESS_CONTENT_EDITABLE_WARNING || propKey === SUPPRESS_HYDRATION_WARNING$1 || // Controlled attributes are not validated
      // TODO: Only ignore them on controlled tags.
...
function warnForDeletedHydratableElement(parentNode, child) {
  {
    ...
    didWarnInvalidHydration = true;
    warningWithoutStack$1('Did not expect server HTML to contain a <%s> in <%s>.', child.nodeName.toLowerCase(), parentNode.nodeName.toLowerCase());
  }
}
function warnForDeletedHydratableText(parentNode, child) {
  {
    ...
    didWarnInvalidHydration = true;
    warningWithoutStack$1('Did not expect server HTML to contain the text node "%s" in <%s>.', child.nodeValue, parentNode.nodeName.toLowerCase());
  }
}
function warnForInsertedHydratedElement(parentNode, tag, props) {
  {
    ...
    didWarnInvalidHydration = true;
    warningWithoutStack$1('Expected server HTML to contain a matching <%s> in <%s>.', tag, parentNode.nodeName.toLowerCase());
  }
}
function warnForInsertedHydratedText(parentNode, text) {
  {
    ...
    didWarnInvalidHydration = true;
    warningWithoutStack$1('Expected server HTML to contain a matching text node for "%s" in <%s>.', text, parentNode.nodeName.toLowerCase());
  }
}
...

{
  ...
  validateDOMNesting = function (childTag, childText, ancestorInfo) {
    ...
    if (childText != null) {
      if (childTag != null) {
        warningWithoutStack$1('validateDOMNesting: when childText is passed, childTag should be null');
      }
    ...
    if (invalidParent) {
      ...
      warningWithoutStack$1('validateDOMNesting(...): %s cannot appear as a child of <%s>.%s%s%s', tagDisplayName, ancestorTag, whitespaceInfo, info, addendum);
    } else {
      warningWithoutStack$1('validateDOMNesting(...): %s cannot appear as a descendant of ' + '<%s>.%s', tagDisplayName, ancestorTag, addendum);
    }
  };
}
...
function dispatchBeforeDetachedBlur(target) {
  ...
  dispatchEventForResponderEventSystem('beforeblur', targetInstance, {
    target: target,
    timeStamp: Date.now()
...
function dispatchDetachedBlur(target) {
  dispatchEventForResponderEventSystem('blur', null, {
...
function beforeRemoveInstance(instance) {
  if (enableFlareAPI && selectionInformation && instance === selectionInformation.focusedElem) {
...
function pop(cursor, fiber) {
...

  {
    if (fiber !== fiberStack[index]) {
      warningWithoutStack$1('Unexpected Fiber popped.');
    }
  }
...

{
  ...
  lowPriorityWarningWithoutStack = function (format) {
    if (format === undefined) {
...

{
  ...
  ReactStrictModeWarnings.flushPendingUnsafeLifecycleWarnings = function () {
    ...
    if (UNSAFE_componentWillMountUniqueNames.size > 0) {
      ...
      warningWithoutStack$1('Using UNSAFE_componentWillMount in strict mode is not recommended and may indicate bugs in your code. ' + 'See https://fb.me/react-unsafe-component-lifecycles for details.\n\n' + '* Move code with side effects to componentDidMount, and set initial state in the constructor.\n' + '\nPlease update the following components: %s', sortedNames);
    }

    if (UNSAFE_componentWillReceivePropsUniqueNames.size > 0) {
      var _sortedNames = setToSortedString(UNSAFE_componentWillReceivePropsUniqueNames);

      warningWithoutStack$1('Using UNSAFE_componentWillReceiveProps in strict mode is not recommended ' + 'and may indicate bugs in your code. ' + 'See https://fb.me/react-unsafe-component-lifecycles for details.\n\n' + '* Move data fetching code or side effects to componentDidUpdate.\n' + "* If you're updating state whenever props change, " + 'refactor your code to use memoization techniques or move it to ' + 'static getDerivedStateFromProps. Learn more at: https://fb.me/react-derived-state\n' + '\nPlease update the following components: %s', _sortedNames);
    }

    if (UNSAFE_componentWillUpdateUniqueNames.size > 0) {
      var _sortedNames2 = setToSortedString(UNSAFE_componentWillUpdateUniqueNames);

      warningWithoutStack$1('Using UNSAFE_componentWillUpdate in strict mode is not recommended ' + 'and may indicate bugs in your code. ' + 'See https://fb.me/react-unsafe-component-lifecycles for details.\n\n' + '* Move data fetching code or side effects to componentDidUpdate.\n' + '\nPlease update the following components: %s', _sortedNames2);
    }

    if (componentWillMountUniqueNames.size > 0) {
      var _sortedNames3 = setToSortedString(componentWillMountUniqueNames);

      lowPriorityWarningWithoutStack$1('componentWillMount has been renamed, and is not recommended for use. ' + 'See https://fb.me/react-unsafe-component-lifecycles for details.\n\n' + '* Move code with side effects to componentDidMount, and set initial state in the constructor.\n' + '* Rename componentWillMount to UNSAFE_componentWillMount to suppress ' + 'this warning in non-strict mode. In React 17.x, only the UNSAFE_ name will work. ' + 'To rename all deprecated lifecycles to their new names, you can run ' + '`npx react-codemod rename-unsafe-lifecycles` in your project source folder.\n' + '\nPlease update the following components: %s', _sortedNames3);
    }

    if (componentWillReceivePropsUniqueNames.size > 0) {
      var _sortedNames4 = setToSortedString(componentWillReceivePropsUniqueNames);

      lowPriorityWarningWithoutStack$1('componentWillReceiveProps has been renamed, and is not recommended for use. ' + 'See https://fb.me/react-unsafe-component-lifecycles for details.\n\n' + '* Move data fetching code or side effects to componentDidUpdate.\n' + "* If you're updating state whenever props change, refactor your " + 'code to use memoization techniques or move it to ' + 'static getDerivedStateFromProps. Learn more at: https://fb.me/react-derived-state\n' + '* Rename componentWillReceiveProps to UNSAFE_componentWillReceiveProps to suppress ' + 'this warning in non-strict mode. In React 17.x, only the UNSAFE_ name will work. ' + 'To rename all deprecated lifecycles to their new names, you can run ' + '`npx react-codemod rename-unsafe-lifecycles` in your project source folder.\n' + '\nPlease update the following components: %s', _sortedNames4);
    }

    if (componentWillUpdateUniqueNames.size > 0) {
      var _sortedNames5 = setToSortedString(componentWillUpdateUniqueNames);

      lowPriorityWarningWithoutStack$1('componentWillUpdate has been renamed, and is not recommended for use. ' + 'See https://fb.me/react-unsafe-component-lifecycles for details.\n\n' + '* Move data fetching code or side effects to componentDidUpdate.\n' + '* Rename componentWillUpdate to UNSAFE_componentWillUpdate to suppress ' + 'this warning in non-strict mode. In React 17.x, only the UNSAFE_ name will work. ' + 'To rename all deprecated lifecycles to their new names, you can run ' + '`npx react-codemod rename-unsafe-lifecycles` in your project source folder.\n' + '\nPlease update the following components: %s', _sortedNames5);
    }
  };
  ...
  ReactStrictModeWarnings.recordLegacyContextWarning = function (fiber, instance) {
    ...
    if (strictRoot === null) {
      warningWithoutStack$1('Expected to find a StrictMode component in a strict mode tree. ' + 'This error is likely caused by a bug in React. Please file an issue.');
      return;
    } // Dedup strategy: Warn once per component.
  ...
  ReactStrictModeWarnings.flushLegacyContextWarning = function () {
    pendingLegacyContextWarning.forEach(function (fiberArray, strictRoot) {
      ...
      warningWithoutStack$1('Legacy context API has been detected within a strict-mode tree.' + '\n\nThe old API will be supported in all 16.x releases, but applications ' + 'using it should migrate to the new version.' + '\n\nPlease update the following components: %s' + '\n\nLearn more about this warning here: https://fb.me/react-legacy-context' + '%s', sortedNames, strictRootComponentStack);
    });
  };
  ...
  if (isPrimaryRenderer) {
...

    {
      if (context._currentRenderer !== undefined && context._currentRenderer !== null && context._currentRenderer !== rendererSigil) {
        warningWithoutStack$1('Detected multiple renderers concurrently rendering the ' + 'same context provider. This is currently unsupported.');
      }
  ...
  if (is$1(oldValue, newValue)) {
...

    {
      if ((changedBits & MAX_SIGNED_31_BIT_INT) !== changedBits) {
        warning$1('calculateChangedBits: Expected the return value to be a ' + '31-bit integer. Instead received: %s', changedBits);
      }
    }
...
function enqueueUpdate(fiber, update) {
...

  {
    if (currentlyProcessingQueue === sharedQueue && !didWarnUpdateInsideUpdate) {
      warningWithoutStack$1('An update (setState, replaceState, or forceUpdate) was scheduled ' + 'from inside an update function. Update functions should be pure, ' + 'with zero side-effects. Consider using componentDidUpdate or a ' + 'callback.');
...

{
  ...
  warnOnInvalidCallback$1 = function (callback, callerName) {
    ...
    if (!didWarnOnInvalidCallback.has(key)) {
      ...
      warningWithoutStack$1('%s(...): Expected the last optional `callback` argument to be a ' + 'function. Instead received: %s.', callerName, callback);
    }
  };
  ...
  warnOnUndefinedDerivedState = function (type, partialState) {
    if (partialState === undefined) {
      ...
      if (!didWarnAboutUndefinedDerivedState.has(componentName)) {
        ...
        warningWithoutStack$1('%s.getDerivedStateFromProps(): A valid state object (or null) must be returned. ' + 'You have returned undefined.', componentName);
      }
    }
...
function checkShouldComponentUpdate(workInProgress, ctor, oldProps, newProps, oldState, newState, nextContext) {
  ...
  if (typeof instance.shouldComponentUpdate === 'function') {
    ...
    {
      if (shouldUpdate === undefined) {
        warningWithoutStack$1('%s.shouldComponentUpdate(): Returned undefined instead of a ' + 'boolean value. Make sure to return true or false.', getComponentName(ctor) || 'Component');
      }
    }
...
function checkClassInstance(workInProgress, ctor, newProps) {
...

  {
    ...
    if (instance.propTypes) {
      warningWithoutStack$1('propTypes was defined as an instance property on %s. Use a static ' + 'property to define propTypes instead.', name);
    }

    if (instance.contextType) {
      warningWithoutStack$1('contextType was defined as an instance property on %s. Use a static ' + 'property to define contextType instead.', name);
    }

    if (disableLegacyContext) {
      if (ctor.childContextTypes) {
        warningWithoutStack$1('%s uses the legacy childContextTypes API which is no longer supported. ' + 'Use React.createContext() instead.', name);
      }

      if (ctor.contextTypes) {
        warningWithoutStack$1('%s uses the legacy contextTypes API which is no longer supported. ' + 'Use React.createContext() with static contextType instead.', name);
      }
    } else {
      if (instance.contextTypes) {
        warningWithoutStack$1('contextTypes was defined as an instance property on %s. Use a static ' + 'property to define contextTypes instead.', name);
      }

      if (ctor.contextType && ctor.contextTypes && !didWarnAboutContextTypeAndContextTypes.has(ctor)) {
        didWarnAboutContextTypeAndContextTypes.add(ctor);
        warningWithoutStack$1('%s declares both contextTypes and contextType static properties. ' + 'The legacy contextTypes property will be ignored.', name);
      }
    }

    if (typeof instance.componentShouldUpdate === 'function') {
      warningWithoutStack$1('%s has a method called ' + 'componentShouldUpdate(). Did you mean shouldComponentUpdate()? ' + 'The name is phrased as a question because the function is ' + 'expected to return a value.', name);
    }
    ...
    if (typeof instance.componentDidUnmount === 'function') {
      warningWithoutStack$1('%s has a method called ' + 'componentDidUnmount(). But there is no such lifecycle method. ' + 'Did you mean componentWillUnmount()?', name);
    }
    ...
    if (typeof instance.componentWillRecieveProps === 'function') {
      warningWithoutStack$1('%s has a method called ' + 'componentWillRecieveProps(). Did you mean componentWillReceiveProps()?', name);
    }

    if (typeof instance.UNSAFE_componentWillRecieveProps === 'function') {
      warningWithoutStack$1('%s has a method called ' + 'UNSAFE_componentWillRecieveProps(). Did you mean UNSAFE_componentWillReceiveProps()?', name);
    }

    var hasMutatedProps = instance.props !== newProps;

    if (instance.props !== undefined && hasMutatedProps) {
      warningWithoutStack$1('%s(...): When calling super() in `%s`, make sure to pass ' + "up the same props that your component's constructor was passed.", name, name);
    }

    if (instance.defaultProps) {
      warningWithoutStack$1('Setting defaultProps as an instance property on %s is not supported and will be ignored.' + ' Instead, define defaultProps as a static property on %s.', name, name);
    }

    if (typeof instance.getSnapshotBeforeUpdate === 'function' && typeof instance.componentDidUpdate !== 'function' && !didWarnAboutGetSnapshotBeforeUpdateWithoutDidUpdate.has(ctor)) {
      didWarnAboutGetSnapshotBeforeUpdateWithoutDidUpdate.add(ctor);
      warningWithoutStack$1('%s: getSnapshotBeforeUpdate() should be used with componentDidUpdate(). ' + 'This component defines getSnapshotBeforeUpdate() only.', getComponentName(ctor));
    }

    if (typeof instance.getDerivedStateFromProps === 'function') {
      warningWithoutStack$1('%s: getDerivedStateFromProps() is defined as an instance method ' + 'and will be ignored. Instead, declare it as a static method.', name);
    }

    if (typeof instance.getDerivedStateFromError === 'function') {
      warningWithoutStack$1('%s: getDerivedStateFromError() is defined as an instance method ' + 'and will be ignored. Instead, declare it as a static method.', name);
    }

    if (typeof ctor.getSnapshotBeforeUpdate === 'function') {
      warningWithoutStack$1('%s: getSnapshotBeforeUpdate() is defined as a static method ' + 'and will be ignored. Instead, declare it as an instance method.', name);
    }
    ...
    if (typeof instance.getChildContext === 'function' && typeof ctor.childContextTypes !== 'object') {
      warningWithoutStack$1('%s.getChildContext(): childContextTypes must be defined in order to ' + 'use getChildContext().', name);
    }
  }
}
    ...
    if (typeof ctor.getDerivedStateFromProps === 'function' || typeof instance.getSnapshotBeforeUpdate === 'function') {
      ...
      if (foundWillMountName !== null || foundWillReceivePropsName !== null || foundWillUpdateName !== null) {
        ...
        if (!didWarnAboutLegacyLifecyclesAndDerivedState.has(_componentName)) {
          ...
          warningWithoutStack$1('Unsafe legacy lifecycles will not be called for components using new component APIs.\n\n' + '%s uses %s but also contains the following legacy lifecycles:%s%s%s\n\n' + 'The above lifecycles should be removed. Learn more about this warning here:\n' + 'https://fb.me/react-unsafe-component-lifecycles', _componentName, newApiName, foundWillMountName !== null ? "\n  " + foundWillMountName : '', foundWillReceivePropsName !== null ? "\n  " + foundWillReceivePropsName : '', foundWillUpdateName !== null ? "\n  " + foundWillUpdateName : '');
        }
      }
    }
  } // Cache unmasked context so we can avoid recreating masked context unless necessary.
  ...
  if (instance.state !== oldState) {
    {
      ...
      if (!didWarnAboutStateAssignmentForComponent.has(componentName)) {
        ...
        warningWithoutStack$1('%s.componentWillReceiveProps(): Assigning directly to ' + "this.state is deprecated (except inside a component's " + 'constructor). Use setState instead.', componentName);
      }
    }
...

  {
    if (instance.state === newProps) {
      ...
      if (!didWarnAboutDirectlyAssigningPropsToState.has(componentName)) {
        ...
        warningWithoutStack$1('%s: It is not recommended to assign props directly to state ' + "because updates to props won't be reflected in state. " + 'In most cases, it is better to use props directly.', componentName);
      }
    }
...

{
  ...
  warnForMissingKey = function (child) {
    ...
    warning$1('Each child in a list should have a unique ' + '"key" prop. See https://fb.me/react-warning-keys for ' + 'more information.');
  };
}
      ...
      if (returnFiber.mode & StrictMode || warnAboutStringRefs) {
        ...
        if (!didWarnAboutStringRefs[componentName]) {
          if (warnAboutStringRefs) {
            warningWithoutStack$1('Component "%s" contains the string ref "%s". Support for string refs ' + 'will be removed in a future major release. We recommend using ' + 'useRef() or createRef() instead. ' + 'Learn more about using refs safely here: ' + 'https://fb.me/react-strict-mode-string-ref%s', componentName, mixedRef, getStackByFiberInDevAndProd(returnFiber));
          } else {
...
function warnOnFunctionType() {
  {
    ...
    warning$1('Functions are not valid as a React child. This may happen if ' + 'you return a Component instead of <Component /> from render. ' + 'Or maybe you meant to call this function rather than return it.');
  }
  ...
  function warnOnInvalidKey(child, knownKeys) {
    {
      ...
      switch (child.$$typeof) {
        ...
        case REACT_PORTAL_TYPE:
          ...
          warning$1('Encountered two children with the same key, `%s`. ' + 'Keys should be unique so that components maintain their identity ' + 'across updates. Non-unique keys may cause children to be ' + 'duplicated and/or omitted — the behavior is unsupported and ' + 'could change in a future version.', key);
          break;
...

    {
      ...
      newChildrenIterable[Symbol.toStringTag] === 'Generator') {
        if (!didWarnAboutGenerators) {
          warning$1('Using Generators as children is unsupported and will likely yield ' + 'unexpected results because enumerating a generator mutates it. ' + 'You may convert it to an array with `Array.from()` or the ' + '`[...spread]` operator before rendering. Keep in mind ' + 'you might need to polyfill these features for older browsers.');
        }
      ...
      if (newChildrenIterable.entries === iteratorFn) {
        if (!didWarnAboutMaps) {
          warning$1('Using Maps as children is unsupported and will likely yield ' + 'unexpected results. Convert it to a sequence/iterable of keyed ' + 'ReactElements instead.');
        }
...
function updateEventListener(listener, fiber, visistedResponders, respondersMap, rootContainerInstance) {
  ...
  if (visistedResponders.has(responder)) {
    // show warning
    {
      warning$1('Duplicate event responder "%s" found in event listeners. ' + 'Event listeners passed to elements cannot use the same event responder more than once.', responder.displayName);
    }
...
function checkDepsAreArrayDev(deps) {
  {
    if (deps !== undefined && deps !== null && !Array.isArray(deps)) {
      ...
      warning$1('%s received a final argument that is not an array (instead, received `%s`). When ' + 'specified, the final argument must be an array.', currentHookNameInDev, typeof deps);
    }
  }
}
...
function warnOnHookMismatchInDev(currentHookName) {
  {
    ...
    if (!didWarnAboutMismatchedHooksForComponent.has(componentName)) {
      ...
      if (hookTypesDev !== null) {
        ...
        warning$1('React has detected a change in the order of Hooks called by %s. ' + 'This will lead to bugs and errors if not fixed. ' + 'For more information, read the Rules of Hooks: https://fb.me/rules-of-hooks\n\n' + '   Previous render            Next render\n' + '   ------------------------------------------------------\n' + '%s' + '   ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n', componentName, table);
      }
    }
  }
}
...
function areHookInputsEqual(nextDeps, prevDeps) {
  ...
  if (prevDeps === null) {
    {
      warning$1('%s received a final argument during this render, but not during ' + 'the previous render. Even though the final argument is optional, ' + 'its type cannot change between renders.', currentHookNameInDev);
    }
  ...
  if (typeof ref === 'function') {
...

    {
      if (!refObject.hasOwnProperty('current')) {
        warning$1('Expected useImperativeHandle() first argument to either be a ' + 'ref callback or React.createRef() object. Instead received: %s.', 'an object with keys {' + Object.keys(refObject).join(', ') + '}');
      }
    }
...
function mountImperativeHandle(ref, create, deps) {
  {
    if (typeof create !== 'function') {
      warning$1('Expected useImperativeHandle() second argument to be a function ' + 'that creates a handle. Instead received: %s.', create !== null ? typeof create : 'null');
    }
...
function dispatchAction(fiber, queue, action) {
...

  {
    if (typeof arguments[3] === 'function') {
      warning$1("State updates from the useState() and useReducer() Hooks don't support the " + 'second callback argument. To execute a side effect after ' + 'rendering, declare it in the component body with useEffect().');
    }
  }
...
function warnIfHydrating() {
  {
    if (isHydrating) {
      warning$1('We should not be hydrating here. This is a bug in React. Please file a bug.');
    }
  }
}
...

  {
    ...
    if (inst.props !== nextProps) {
      if (!didWarnAboutReassigningProps) {
        warning$1('It looks like %s is reassigning its own `this.props` while rendering. ' + 'This is not supported and can lead to confusing bugs.', getComponentName(workInProgress.type) || 'a component');
      }
...

  {
    if (Component.prototype && typeof Component.prototype.render === 'function') {
      ...
      if (!didWarnAboutBadClass[componentName]) {
        warningWithoutStack$1("The <%s /> component appears to have a render method, but doesn't extend React.Component. " + 'This is likely to cause errors. Change %s to extend React.Component instead.', componentName, componentName);
  ...
  if (typeof value === 'object' && value !== null && typeof value.render === 'function' && value.$$typeof === undefined) {
...

    {
      if (disableLegacyContext && Component.contextTypes) {
        warningWithoutStack$1('%s uses the legacy contextTypes API which is no longer supported. ' + 'Use React.createContext() with React.useContext() instead.', getComponentName(Component) || 'Unknown');
      }
...
function validateFunctionComponentInDev(workInProgress, Component) {
  {
    if (Component) {
      if (Component.childContextTypes) {
        warningWithoutStack$1('%s(...): childContextTypes cannot be defined on a function component.', Component.displayName || Component.name || 'Component');
      }
    }
    ...
    if (workInProgress.ref !== null) {
      ...
      if (!didWarnAboutFunctionRefs[warningKey]) {
        ...
        warning$1('Function components cannot be given refs. ' + 'Attempts to access this ref will fail. ' + 'Did you mean to use React.forwardRef()?%s', info);
      }
    }
    ...
    if (warnAboutDefaultPropsOnFunctionComponents && Component.defaultProps !== undefined) {
      ...
      if (!didWarnAboutDefaultPropsOnFunctionComponent[componentName]) {
        warningWithoutStack$1('%s: Support for defaultProps will be removed from function components ' + 'in a future major release. Use JavaScript default parameters instead.', componentName);
    ...
    if (typeof Component.getDerivedStateFromProps === 'function') {
      ...
      if (!didWarnAboutGetDerivedStateOnFunctionComponent[_componentName2]) {
        warningWithoutStack$1('%s: Function components do not support getDerivedStateFromProps.', _componentName2);
    ...
    if (typeof Component.contextType === 'object' && Component.contextType !== null) {
      ...
      if (!didWarnAboutContextTypeOnFunctionComponent[_componentName3]) {
        warningWithoutStack$1('%s: Function components do not support contextType.', _componentName3);
...

  {
    if ('maxDuration' in nextProps) {
      if (!didWarnAboutMaxDuration) {
        ...
        warning$1('maxDuration has been removed from React. ' + 'Remove the maxDuration prop.');
      }
    }
...
function mountDehydratedSuspenseComponent(workInProgress, suspenseInstance, renderExpirationTime) {
  ...
  if ((workInProgress.mode & BlockingMode) === NoMode) {
    {
      warning$1('Cannot hydrate Suspense in legacy mode. Switch from ' + 'ReactDOM.hydrate(element, container) to ' + 'ReactDOM.createBlockingRoot(container, { hydrate: true })' + '.render(element) or remove the Suspense components from ' + 'the server rendered components.');
    }
    ...
    if (revealOrder !== undefined && revealOrder !== 'forwards' && revealOrder !== 'backwards' && revealOrder !== 'together' && !didWarnAboutRevealOrder[revealOrder]) {
      ...
      if (typeof revealOrder === 'string') {
        switch (revealOrder.toLowerCase()) {
          ...
          case 'backwards':
            {
              warning$1('"%s" is not a valid value for revealOrder on <SuspenseList />. ' + 'Use lowercase "%s" instead.', revealOrder, revealOrder.toLowerCase());
              break;
            }
          ...
          case 'backward':
            {
              warning$1('"%s" is not a valid value for revealOrder on <SuspenseList />. ' + 'React uses the -s suffix in the spelling. Use "%ss" instead.', revealOrder, revealOrder.toLowerCase());
              break;
            }

          default:
            warning$1('"%s" is not a supported revealOrder on <SuspenseList />. ' + 'Did you mean "together", "forwards" or "backwards"?', revealOrder);
            break;
        }
      } else {
        warning$1('%s is not a supported value for revealOrder on <SuspenseList />. ' + 'Did you mean "together", "forwards" or "backwards"?', revealOrder);
      }
    }
  }
}
...
function validateTailOptions(tailMode, revealOrder) {
  {
    if (tailMode !== undefined && !didWarnAboutTailOptions[tailMode]) {
      if (tailMode !== 'collapsed' && tailMode !== 'hidden') {
        ...
        warning$1('"%s" is not a supported value for tail on <SuspenseList />. ' + 'Did you mean "collapsed" or "hidden"?', tailMode);
      } else if (revealOrder !== 'forwards' && revealOrder !== 'backwards') {
        didWarnAboutTailOptions[tailMode] = true;
        warning$1('<SuspenseList tail="%s" /> is only valid if revealOrder is ' + '"forwards" or "backwards". ' + 'Did you mean to specify revealOrder="forwards"?', tailMode);
      }
    }
  }
}
...
function validateSuspenseListNestedChild(childSlot, index) {
  {
    ...
    if (isArray || isIterable) {
      ...
      warning$1('A nested %s was passed to row #%s in <SuspenseList />. Wrap it in ' + 'an additional SuspenseList to configure its revealOrder: ' + '<SuspenseList revealOrder=...> ... ' + '<SuspenseList revealOrder=...>{%s}</SuspenseList> ... ' + '</SuspenseList>', type, index, type);
      return false;
    }
  }
    ...
    if ((revealOrder === 'forwards' || revealOrder === 'backwards') && children !== undefined && children !== null && children !== false) {
      if (Array.isArray(children)) {
        ...
        if (typeof iteratorFn === 'function') {
          ...
          warning$1('A single row was passed to a <SuspenseList revealOrder="%s" />. ' + 'This is not useful since it needs multiple rows. ' + 'Did you mean to pass multiple children or an array?', revealOrder);
        }
      }
    }
  }
    ...
    if (context._context === undefined) {
      ...
      if (context !== context.Consumer) {
        if (!hasWarnedAboutUsingContextAsConsumer) {
          ...
          warning$1('Rendering <Context> directly is not supported and will be removed in ' + 'a future major release. Did you mean to render <Context.Consumer> instead?');
        }
      }
    } else {
          ...
          if (current$$1 !== null) {
...

            {
              ...
              if (snapshot === undefined && !didWarnSet.has(finishedWork.type)) {
                ...
                warningWithoutStack$1('%s.getSnapshotBeforeUpdate(): A snapshot value (or null) ' + 'must be returned. You have returned undefined.', getComponentName(finishedWork.type));
              }
            }
    ...
    if (typeof ref === 'function') {
    ...
    } else {
      {
        if (!ref.hasOwnProperty('current')) {
          warningWithoutStack$1('Unexpected ref object provided for %s. ' + 'Use either a ref-setter function or React.createRef().%s', getComponentName(finishedWork.type), getStackByFiberInDevAndProd(finishedWork));
        }
      }
...
function commitUnmount(finishedRoot, current$$1, renderPriorityLevel) {
  ...
  switch (current$$1.tag) {
    ...
    case HostComponent:
      {
        if (enableFlareAPI) {
          unmountResponderListeners(current$$1);
    ...
    case HostComponent:
      {
        ...
        if (instance != null) {
          ...
          if (enableFlareAPI) {
            ...
            if (prevListeners !== nextListeners || current$$1 === null) {
        ...
        if (enableScopeAPI) {
          ...
          if (enableFlareAPI) {
            ...
            if (_prevListeners !== _nextListeners || current$$1 === null) {
              updateLegacyEventListeners(_nextListeners, finishedWork, null);
            }
          }
        }
  ...
  if (enableSuspenseCallback && newState !== null) {
    ...
    if (typeof suspenseCallback === 'function') {
      ...
      if (suspenseCallback !== undefined) {
        warning$1('Unexpected type for suspenseCallback.');
      }
    }
  }
}
...
function flushDiscreteUpdates() {
  ...
  if ((executionContext & (BatchedContext | RenderContext | CommitContext)) !== NoContext) {
    {
      if ((executionContext & RenderContext) !== NoContext) {
        warning$1('unstable_flushDiscreteUpdates: Cannot flush updates when React is ' + 'already rendering.');
      }
...
function checkForNestedUpdates() {
...

  {
    if (nestedPassiveUpdateCount > NESTED_PASSIVE_UPDATE_LIMIT) {
      ...
      warning$1('Maximum update depth exceeded. This can happen when a component ' + "calls setState inside useEffect, but useEffect either doesn't " + 'have a dependency array, or one of the dependencies changes on ' + 'every render.');
    }
  }
}
...
function warnAboutUpdateOnUnmountedFiberInDEV(fiber) {
  {
    ...
    warningWithoutStack$1("Can't perform a React state update on an unmounted component. This " + 'is a no-op, but it indicates a memory leak in your application. To ' + 'fix, cancel all subscriptions and asynchronous tasks in %s.%s', tag === ClassComponent ? 'the componentWillUnmount method' : 'a useEffect cleanup function', getStackByFiberInDevAndProd(fiber));
  }
}
    ...
    if (fiber.tag === ClassComponent) {
      switch (phase) {
        ...
        case 'render':
          ...
          warningWithoutStack$1('Cannot update during an existing state transition (such as ' + 'within `render`). Render methods should be a pure function of ' + 'props and state.');
...
function warnIfNotScopedWithMatchingAct(fiber) {
  {
    if (warnsIfNotActing === true && IsSomeRendererActing.current === true && IsThisRendererActing.current !== true) {
      warningWithoutStack$1("It looks like you're using the wrong act() around your test interactions.\n" + 'Be sure to use the matching version of act() corresponding to your renderer:\n\n' + '// for react-dom:\n' + "import {act} from 'react-dom/test-utils';\n" + '// ...\n' + 'act(() => ...);\n\n' + '// for react-test-renderer:\n' + "import TestRenderer from 'react-test-renderer';\n" + 'const {act} = TestRenderer;\n' + '// ...\n' + 'act(() => ...);' + '%s', getStackByFiberInDevAndProd(fiber));
    }
  }
}
function warnIfNotCurrentlyActingEffectsInDEV(fiber) {
  {
    if (warnsIfNotActing === true && (fiber.mode & StrictMode) !== NoMode && IsSomeRendererActing.current === false && IsThisRendererActing.current === false) {
      warningWithoutStack$1('An update to %s ran an effect, but was not wrapped in act(...).\n\n' + 'When testing, code that causes React state updates should be ' + 'wrapped into act(...):\n\n' + 'act(() => {\n' + '  /* fire events that update state */\n' + '});\n' + '/* assert on the output */\n\n' + "This ensures that you're testing the behavior the user would see " + 'in the browser.' + ' Learn more at https://fb.me/react-wrap-tests-with-act' + '%s', getComponentName(fiber.type), getStackByFiberInDevAndProd(fiber));
    }
  }
}
...
function warnIfNotCurrentlyActingUpdatesInDEV(fiber) {
  {
    if (warnsIfNotActing === true && executionContext === NoContext && IsSomeRendererActing.current === false && IsThisRendererActing.current === false) {
      warningWithoutStack$1('An update to %s inside a test was not wrapped in act(...).\n\n' + 'When testing, code that causes React state updates should be ' + 'wrapped into act(...):\n\n' + 'act(() => {\n' + '  /* fire events that update state */\n' + '});\n' + '/* assert on the output */\n\n' + "This ensures that you're testing the behavior the user would see " + 'in the browser.' + ' Learn more at https://fb.me/react-wrap-tests-with-act' + '%s', getComponentName(fiber.type), getStackByFiberInDevAndProd(fiber));
    }
  }
}
...
function warnIfUnmockedScheduler(fiber) {
  {
    if (didWarnAboutUnmockedScheduler === false && unstable_flushAllWithoutAsserting === undefined) {
      if (fiber.mode & BlockingMode || fiber.mode & ConcurrentMode) {
        ...
        warningWithoutStack$1('In Concurrent or Sync modes, the "scheduler" module needs to be mocked ' + 'to guarantee consistent behaviour across tests and browsers. ' + 'For example, with jest: \n' + "jest.mock('scheduler', () => require('scheduler/unstable_mock'));\n\n" + 'For more info, visit https://fb.me/react-mock-scheduler');
      } else if (warnAboutUnmockedScheduler === true) {
        didWarnAboutUnmockedScheduler = true;
        warningWithoutStack$1('Starting from React v17, the "scheduler" module will need to be mocked ' + 'to guarantee consistent behaviour across tests and browsers. ' + 'For example, with jest: \n' + "jest.mock('scheduler', () => require('scheduler/unstable_mock'));\n\n" + 'For more info, visit https://fb.me/react-mock-scheduler');
      }
    }
  }
}
...
function injectInternals(internals) {
  ...
  if (!hook.supportsFiber) {
    {
      warningWithoutStack$1('The installed version of React DevTools is too old and will not work ' + 'with the current version of React. Please update React DevTools. ' + 'https://fb.me/react-devtools');
        ...
        onScheduleFiberRoot = function (root, children) {
          ...
          try {
            ...
            if (true && !hasLoggedError) {
              hasLoggedError = true;
              warningWithoutStack$1('React instrumentation encountered an error: %s', err);
            }
          }
        };
      }
...
function createFiberRoot(containerInfo, tag, hydrate, hydrationCallbacks) {
  debugger;
  // 纯个人理解，根fiber
  // 两个fiber，一个是创建fiberRootNode，一个是创建fiberNode
    ...
    if (hostFiber.mode & StrictMode) {
      ...
      if (!didWarnAboutFindNodeInStrictMode[componentName]) {
        ...
        if (fiber.mode & StrictMode) {
          warningWithoutStack$1('%s is deprecated in StrictMode. ' + '%s was passed an instance of %s which is inside StrictMode. ' + 'Instead, add a ref directly to the element you want to reference. ' + 'Learn more about using refs safely here: ' + 'https://fb.me/react-strict-mode-find-node%s', methodName, methodName, componentName, getStackByFiberInDevAndProd(hostFiber));
        } else {
          warningWithoutStack$1('%s is deprecated in StrictMode. ' + '%s was passed an instance of %s which renders StrictMode children. ' + 'Instead, add a ref directly to the element you want to reference. ' + 'Learn more about using refs safely here: ' + 'https://fb.me/react-strict-mode-find-node%s', methodName, methodName, componentName, getStackByFiberInDevAndProd(hostFiber));
        }
      }
    }
...
function updateContainer(element, container, parentComponent, callback) {
...

  {
    if (phase === 'render' && current !== null && !didWarnAboutNestedUpdates) {
      ...
      warningWithoutStack$1('Render methods should be a pure function of props and state; ' + 'triggering nested component updates from render is not allowed. ' + 'If necessary, trigger nested updates in componentDidUpdate.\n\n' + 'Check the render method of %s.', getComponentName(current.type) || 'Unknown');
    }
  }

  // 所以说这个时间是给 react devtools准备的？
  // 感觉应该分开比较好，咋能和核心代码耦合在一块呢？以插件的形式放进来是不是更好？
  ...
  if (callback !== null) {
    {
      if (typeof callback !== 'function') {
        warningWithoutStack$1('render(...): Expected the last optional `callback` argument to be a ' + 'function. Instead received: %s.', callback);
      }
    }
...
function getPublicRootInstance(container) {
  // 获取之前设置的current，也就是那个用来初始化的fiber
  debugger;
...
function createRootImpl(container, tag, options) {
  ...
  // 在render的container上挂载root.current
...
function warnIfReactDOMContainerInDEV(container) {
  {
    if (isContainerMarkedAsRoot(container)) {
      if (container._reactRootContainer) {
        warningWithoutStack$1('You are calling ReactDOM.createRoot() on a container that was previously ' + 'passed to ReactDOM.render(). This is not supported.');
      } else {
        warningWithoutStack$1('You are calling ReactDOM.createRoot() on a container that ' + 'has already been passed to createRoot() before. Instead, call ' + 'root.render() on the existing root instead if you want to update it.');
      }
    }
  }
}
...

{
  topLevelUpdateWarnings = function (container) {
    debugger;
    // 如果是_reactRootContainer 并且不是 注释
    if (container._reactRootContainer && container.nodeType !== COMMENT_NODE) {
      var hostInstance = findHostInstanceWithNoPortals(container._reactRootContainer._internalRoot.current);

      if (hostInstance) {
        if (hostInstance.parentNode !== container) {
          warningWithoutStack$1('render(...): It looks like the React-rendered content of this ' + 'container was removed without using React. This is not ' + 'supported and will cause errors. Instead, call ' + 'ReactDOM.unmountComponentAtNode to empty a container.');
        }
      }
    }
    ...
    if (hasNonRootReactChild && !isRootRenderedBySomeReact) {
      warningWithoutStack$1('render(...): Replacing React-rendered children with a new root ' + 'component. If you intended to update the children of this node, ' + 'you should instead have the existing children update their state ' + 'and render the new components instead of calling ReactDOM.render.');
    }

    // 不让渲染在body上
    if (container.nodeType === ELEMENT_NODE && container.tagName && container.tagName.toUpperCase() === 'BODY') {
      warningWithoutStack$1('render(): Rendering components directly into document.body is ' + 'discouraged, since its children are often manipulated by third-party ' + 'scripts and browser extensions. This may lead to subtle ' + 'reconciliation issues. Try rendering into a container element created ' + 'for your app.');
    }
  };
}
...
function legacyCreateRootFromDOMContainer(container, forceHydrate) {
    ...
    while (rootSibling = container.lastChild) {
      {
        if (!warned && rootSibling.nodeType === ELEMENT_NODE && rootSibling.hasAttribute(ROOT_ATTRIBUTE_NAME)) {
          warned = true;
          warningWithoutStack$1('render(): Target node has markup rendered by React, but there ' + 'are unrelated nodes as well. This is most commonly caused by ' + 'white-space inserted around server-rendered markup.');
        }
      }
...

  {
    if (shouldHydrate && !forceHydrate && !warnedAboutHydrateAPI) {
      ...
      lowPriorityWarningWithoutStack$1('render(): Calling ReactDOM.render() to hydrate server-rendered markup ' + 'will stop working in React v17. Replace the ReactDOM.render() call ' + 'with ReactDOM.hydrate() if you want React to attach to the server HTML.');
    }
  }
...
// legacyRenderSubtreeIntoContainer(null, element, container, false, callback) 示例调用方式
// 怎么大型ui框架，永远是绕来绕去捏。。。。
function legacyRenderSubtreeIntoContainer(parentComponent, children, container, forceHydrate, callback) {
  ...
  if (!root) {
    // 第一次render是没有_reactRootContaier的，所以当时首次render时候，会赋值一个
    // Initial mount
...
function findDOMNode(componentOrElement) {
  {
    ...
    if (owner !== null && owner.stateNode !== null) {
      ...
      if (!warnedAboutRefsInRender) {
        warningWithoutStack$1('%s is accessing findDOMNode inside its render(). ' + 'render() should be a pure function of props and state. It should ' + 'never access something that requires stale data from the previous ' + 'render, such as refs. Move this logic to componentDidMount and ' + 'componentDidUpdate instead.', getComponentName(owner.type) || 'A component');
      }
...
function hydrate(element, container, callback) {
...

  {
    ...
    if (isModernRoot) {
      warningWithoutStack$1('You are calling ReactDOM.hydrate() on a container that was previously ' + 'passed to ReactDOM.createRoot(). This is not supported. ' + 'Did you mean to call createRoot(container, {hydrate: true}).render(element)?');
    }
...
function render(element, container, callback) {
...

  {
    ...
    if (isModernRoot) {
      warningWithoutStack$1('You are calling ReactDOM.render() on a container that was previously ' + 'passed to ReactDOM.createRoot(). This is not supported. ' + 'Did you mean to call root.render(element)?');
    }
  }
...
function unmountComponentAtNode(container) {
...

  {
    ...
    if (isModernRoot) {
      warningWithoutStack$1('You are calling ReactDOM.unmountComponentAtNode() on a container that was previously ' + 'passed to ReactDOM.createRoot(). This is not supported. Did you mean to call root.unmount()?');
    }
  }
  ...
  if (container._reactRootContainer) {
    {
      ...
      if (renderedByDifferentReact) {
        warningWithoutStack$1("unmountComponentAtNode(): The node you're attempting to unmount " + 'was rendered by another copy of React.');
      }
...

{
  ...
  Set.prototype == null || typeof Set.prototype.clear !== 'function' || typeof Set.prototype.forEach !== 'function') {
    warningWithoutStack$1('React depends on Map and Set built-in types. Make sure that you load a ' + 'polyfill in older browsers. https://fb.me/react-polyfills');
  }
}
  ...
  unstable_createPortal: function () {
    {
      if (!didWarnAboutUnstableCreatePortal) {
        ...
        lowPriorityWarningWithoutStack$1('The ReactDOM.unstable_createPortal() alias has been deprecated, ' + 'and will be removed in React 17+. Update your code to use ' + 'ReactDOM.createPortal() instead. It has the exact same API, ' + 'but without the "unstable_" prefix.');
      }
    }
