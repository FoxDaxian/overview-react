...
function warningWithoutStack(format) {
  for (
  ...
  return warningWWW.apply(void 0, [false, format].concat(args));
}
...

{
  warning = function(format) {
    ...
    warningWithoutStack.apply(void 0, [format + "%s"].concat(args, [stack]));
  };
}
...
var enableSelectiveHydration = _require.enableSelectiveHydration;
var enableTrainModelFix = _require.enableTrainModelFix; // In www, we have experimental support for gathering data
...
function setComponentTree(
  ...
  {
    if (!getNodeFromInstance$1 || !getInstanceFromNode$2) {
      warningWithoutStack(
...
// This refers to a WWW module.
var lowPriorityWarningWWW = require("lowPriorityWarning");

function lowPriorityWarningWithoutStack(format) {
  for (
    var _len = arguments.length,
      args = new Array(_len > 1 ? _len - 1 : 0),
      _key = 1;
  ...
  return lowPriorityWarningWWW.apply(void 0, [false, format].concat(args));
}
