        ...
        if (!state.isActive) {
          ...
          var buttons =
            nativeEvent.button === 1
              ...
              : nativeEvent.buttons;
          var activateAuxiliary = isAuxiliary(buttons, event);
  ...
  onRootEvent: function(event, context, props, state) {
    ...
    switch (eventType) {
      ...
      case "scroll": {
        if (
          state.isActive &&
          state.responderTarget != null && // We ignore incoming scroll events when using mouse events
