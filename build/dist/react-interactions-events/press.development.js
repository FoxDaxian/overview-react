...
function createGestureState(e, type) {
  return {
    ...
    preventDefault: function () {
      ...
      {
        warning$1('preventDefault is not available on event objects created from event responder modules (React Flare). ' + 'Try wrapping in a conditional, i.e. `if (event.type !== "press") { event.preventDefault() }`');
      }
    },
    stopPropagation: function () {
      // NO-OP, we should remove this in the future
      {
        warning$1('stopPropagation is not available on event objects created from event responder modules (React Flare). ' + 'Try wrapping in a conditional, i.e. `if (event.type !== "press") { event.stopPropagation() }`');
      }
    }
  };
}
