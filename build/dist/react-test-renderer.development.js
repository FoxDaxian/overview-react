...
function appendChild(parentInstance, child) {
  {
    if (!Array.isArray(parentInstance.children)) {
      warning$1('An invalid container has been provided. ' + 'This may indicate that another renderer is being used in addition to the test renderer. ' + '(For example, ReactDOM.createPortal inside of a ReactTestRenderer tree.) ' + 'This is not supported.');
    }
  }
  ...
  if (enableFlareAPI) {
    if (props.DEPRECATED_flareListeners != null) {
      ...
      var DEPRECATED_flareListeners = props.DEPRECATED_flareListeners,
          otherProps = _objectWithoutPropertiesLoose(props, ["DEPRECATED_flareListeners"]); // eslint-disable-line
...
function createTextInstance(text, rootContainerInstance, hostContext, internalInstanceHandle) {
  {
    if (enableFlareAPI) {
      if (hostContext === EVENT_COMPONENT_CONTEXT) {
        warning$1('validateDOMNesting: React event components cannot have text DOM nodes as children. ' + 'Wrap the child text "%s" in an element.', text);
      }
    }
  }
  ...
  enqueueTask = function (callback) {
    {
      if (didWarnAboutMessageChannel === false) {
        ...
        if (typeof MessageChannel === 'undefined') {
          warningWithoutStack$1('This browser does not have a MessageChannel implementation, ' + 'so enqueuing tasks via await act(async () => ...) will fail. ' + 'Please file an issue at https://github.com/facebook/react/issues ' + 'if you encounter this warning.');
        }
      }
    }
...
function act(callback) {
  ...
  function onDone() {
...

    {
      if (actingUpdatesScopeDepth > previousActingUpdatesScopeDepth) {
        ...
        warningWithoutStack$1('You seem to have overlapping act() calls, this is not supported. ' + 'Be sure to await previous act() calls before making a new one. ');
      }
    }
  }
  ...
  if (result !== null && typeof result === 'object' && typeof result.then === 'function') {
...

    {
      if (typeof Promise !== 'undefined') {
        ...
        Promise.resolve().then(function () {}).then(function () {
          if (called === false) {
            warningWithoutStack$1('You called act(async () => ...) without await. ' + 'This could lead to unexpected testing behaviour, interleaving multiple act ' + 'calls and mixing their scopes. You should - await act(async () => ...);');
          }
        });
      }
    ...
    {
      if (result !== undefined) {
        warningWithoutStack$1('The callback passed to act(...) function ' + 'must return undefined, or a Promise. You returned %s', result);
      }
    ...
    return {
      then: function (resolve) {
        {
          warningWithoutStack$1('Do not await the result of calling act(...) with sync logic, it is not a Promise.');
        }
